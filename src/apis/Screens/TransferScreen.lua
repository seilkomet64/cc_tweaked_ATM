...
local function createScreen(args)
    ...
    function confirmButton()
        local success, balance = pcall(function() return atmAPI.transfer(args.acc, args.targetAcc, finalAmount, args.pin) end)
        finalAmount = 0
        if not success then
            -- Handle the error
    ...
    function submitFunction()
        if amount == 0 then
            errorDialog(main, mainContent, {"You can't transfer nothing!"})
        elseif newBalance < 0 then
            errorDialog(main, mainContent, {"You don't have enough " .. CONFIG.COINNAME .."!"})
        else
            finalAmount = amount
            confirmDialog(main, mainContent, string.format("Transfer %s %ss to %s?", finalAmount, CONFIG.COINNAME, args.targetAcc), confirmButton)
        end
    end

    main = selectionBase("Transfer\n Account " .. args.targetAcc, submitFunction, args)
    ...
    -- Add Labels
    labelRow:addLabel():setText("How many " .. CONFIG.COINNAME .. "s do you want to transfer?"):setPosition("parent.w * 0.5 + 1 - self.w * 0.5", "parent.h - 1")
    ...
    buttonRow:addButton():setText("-50"):setSize(buttonSize[1], buttonSize[2]):setPosition(2):onClick(function() updateLabel(-50) end)
    buttonRow:addButton():setText("-10"):setSize(buttonSize[1], buttonSize[2]):setPosition(8):onClick(function() updateLabel(-10) end)
    ...
    buttonRow:addButton():setText("+10"):setSize(buttonSize[1], buttonSize[2]):setPosition("parent.w - 11"):onClick(function() updateLabel(10) end)
    buttonRow:addButton():setText("+50"):setSize(buttonSize[1], buttonSize[2]):setPosition("parent.w - 5"):onClick(function() updateLabel(50) end)
